...
export const CategoryUseful =({BuildOnUsefulLands,EmptyTypes,FarmTypes,OtherTypes,LiveTypes,TypeName}) => {
    ...
    if (BuildOnUsefulLands.length >0) {//กรณีมีสิ่งปลุกสร้าง
     ...
     if (LiveTypes.length === 0 &&OtherTypes.length === 0&&FarmTypes.length === 0&& EmptyTypes.length === 0) {
         ...
         return <>
                 {LiveTypes.length>0&&LiveTypes.map((live,i)=>(<p key={i}>{`อยู่อาศัย(${live.Live_Status?"หลัก":"รอง"})`}</p>))}
                 {OtherTypes.length>0&&OtherTypes.map((other,i)=>(<p key={i}>อื่นๆ</p> ))}    
                 {FarmTypes.length>0&&FarmTypes.map((farm,i)=>(<p key={i}>เกษตร</p>))}    
                 {EmptyTypes.length>0&&EmptyTypes.map((empty,i)=><p key={i}>ว่างเปล่า</p> )}    
